    ...
    public static List<NameValuePair> parse(CharArrayBuffer charArrayBuffer, Charset charset, char... cArr) {
        ...
        while (!parserCursor.atEnd()) {
            ...
            if (!parserCursor.atEnd()) {
                ...
                if (cCharAt == '=') {
                    bitSet.clear(61);
                    token2 = tokenParser.parseToken(charArrayBuffer, parserCursor, bitSet);
                    if (!parserCursor.atEnd()) {
                        parserCursor.updatePos(parserCursor.getPos() + 1);
                    }
                }
            ...
            if (!token.isEmpty()) {
                arrayList.add(new BasicNameValuePair(decodeFormFields(token, charset), decodeFormFields(token2, charset)));
            }
        }
        return arrayList;
    }
